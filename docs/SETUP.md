...
- **Daily automatic refresh** (Mac): `bash ops/install_schedule.sh` sets
  ...
  how to remove it.
- **European zones**: the dashboard covers seven European markets, but
  viewing them is not automatic. The zone data is generated by the ETL
  and is not shipped in the repo, so a fresh clone starts GB-only until
  you fetch it yourself with a free ENTSO-E API token — see the README's
  "European zones (ENTSO-E)" section for the three-step setup. Without a
  token the app still runs — the switcher simply stays GB-only.
