...
**See it live: <https://lptva.github.io/gb-power-dashboard/>** – updated daily, nothing to install.
...
Just want to look? Use the [live link](https://lptva.github.io/gb-power-dashboard/) above – everything below is for running the dashboard yourself, with your own data pipeline.
...
**The core dashboard needs no subscription of any kind.** Every data source is free (see the table below; everything the GB view uses is keyless, and the token that refreshes the European zones is free too), and every panel – the year of half-hourly data, the merit-order model, spreads, flows, the seven European zones, the scheduled daily refresh (both the hosted site's daily rebuild and a local scheduled job) works without any
account.
...
**Claude-only by design, not an oversight.** The step invokes the Claude Code CLI as a version-controlled agent (`.claude/agents/dashboard-watcher.md`) on a Claude subscription rather than a metered API key, and the publish validator, the cost logging and the retry logic all parse that CLI's own JSON result envelope. Another provider would need a second auth model, a second envelope format and the publish guards re-validated against its failure modes – not a swapped API key. Assessed in [issue #29](https://github.com/lptva/gb-power-dashboard/issues/29) and judged not worth the maintenance for an optional, default-off panel.
...
**Generating the summary and showing it are two separate opt-ins.** On top of the pipeline flag above, the dashboard itself has an **"AI interpretation"** switch on the summary panel that controls only whether an already-generated summary is rendered. It defaults to **off for every visitor on every load**, and the choice is held in memory only – no cookie, no localStorage, consistent with the project's no-browser-storage rule – so enabling it and reloading returns it to off. It is a display switch, not a data switch: the summary is still generated and published to `data/overnight_summary.json` regardless of the switch's state, and anyone can fetch that file directly – "off" means "not shown by default", not "not fetched". The switch's own tooltip says exactly that.
...
The dashboard covers seven European markets behind the header zone switcher. On the [hosted dashboard](https://lptva.github.io/gb-power-dashboard/) they are already there — nothing to install, nothing to configure.

Running your own copy is different: the zone data is generated by the ETL and is not shipped in the repo, so you need a free ENTSO-E Transparency Platform token to fetch it.
...
3. Fetch a zone with `python3 etl/fetch_entsoe.py --zone FR --days 30`, or let the scheduled refresh keep all seven topped up (zone history accumulates append-only from late May 2026).
...
- **One third-party runtime request** – Cloudflare Web Analytics (cookieless, so the storage rule holds; gated off dev origins so local runs never fire it). It is the only resource the app requests from any domain other than its own host; everything else, ECharts included, is vendored. Vendored ECharts provenance: v5.6.0 (zrender 5.5.1), Apache-2.0, official npm dist, sha256 `e84270bd0cd5bdf60fefc26d00c2a391cb2e81f4d26a7a9ee16185a54773a3cf`.
...
Stdlib `unittest`, no dependencies – consistent with the rest of the project; the same suite runs on every push via GitHub Actions (the badge at the top of this page). Eleven suites now; the founding four are anchored on the places that had already caught real bugs: `tests/test_merit_panel_figures.py` checks `ops/merit_panel_figures.py` against the app's own `metrics.js` outputs (captured in a real browser) on three real-data fixtures — this is the guard that stops the overnight LLM inventing SRMC figures, so it gets its own safety net; `tests/test_overnight_validator.py` pins the publish gate, including the assumption-vocabulary regex against the verbatim sentence from the run that motivated it; `tests/test_panel_facts.py` verifies the precomputed summary statistics against a hand-calculated synthetic dataset; `tests/test_env_flags.py` pins the AI summary's opt-in default (absent means off) and its environment-over-`.env` precedence.
...
- **The BESS calculator's toll/financing block is assumption arithmetic** laid over the observed merchant anchor – no lender downside case, no balloon or refinancing, no tax. Toll price, gearing and cost of debt are figures the user types, not figures the dashboard measured.- **The LDES mini cap-and-floor calculator is indicative ex-tax arithmetic, not Ofgem's CFFM** – no corporation-tax annuity (genuine levels sit above the ex-tax ones), no Repex or ACOD floor, and per-project Ofgem levels are unpublished, so nothing it computes can be validated against a real award; verified against Ofgem's CFFM v2.17 - remaining gap is the documented tax omission (roughly 10-13%).
