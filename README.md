...
The dashboard covers seven European markets behind the header zone switcher. Viewing them is not automatic: the zone data is generated by the ETL and is not shipped in the repo, so a fresh clone starts GB-only until you fetch it yourself with a free ENTSO-E Transparency Platform token.
