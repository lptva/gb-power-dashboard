...
**Mini-CFFM levels** (Assumption, LDES tab — `Metrics.cffmLevels`, mirrored in `ops/ldes_cffm_figures.py`):
    ...
    RAV         = preOpRAV + tx
    dep         = (RAV − residual) / opYears                              (straight line, to the residual)
    return_n    = r · openingRAV_n
    AF(r)       = r / (1 − (1 + r)^−opYears)                              (A1.151; r = 0 → 1/opYears)
    level(r)    = AF(r) · Σ_n (opex + decom + dep + return_n) · (1 + r)^−n

Computed twice, at the floor rate and the cap rate (indicative 4.47% / 7.48% CPIH-real prefilled, refixed at each project's FID), everything flat real £m and **ex-tax** — the real CFFM adds a grossed-up nominal corporation-tax annuity, so genuine levels sit above these. No Repex and no ACOD floor. The closed form the test suite pins: `level(r) = (RAV − residual·(1+r)^−N)·AF(r) + opex + decom`, which with every side block off degenerates to the annuity identity `level = RAV × AF(r)`. The return base deviates from the handbook's A1.143 (see judgement call 18). One caveat from first real use: the decommissioning input is the CFFM's **annual** baseline allowance, charged every operational year — a one-off end-of-life cost entered there is silently multiplied by the regime length (25× at the default). A one-off £X at the end of the regime belongs in the field as its annuitised equivalent, X·(1+r)^−N·AF(r, N) per year at the floor rate, and the card's live line under the field states the lifetime total and computes that equivalent (`Metrics.cffmAnnuitiseEndOfLife`, mirrored in `ops/ldes_cffm_figures.py`).
...
18. **The mini-CFFM is ex-tax, and its return base deviates from the handbook's, both deliberately.** Ex-tax (D74) because the CFFM's corporation-tax loop is a grossed-up *nominal* annuity iterated against capital allowances — replicating it needs an inflation assumption and a tax model this flat-real dashboard has neither of, and a half-replicated tax loop would be worse than a loudly stated omission (the card, the in-app methodology and the formula entry above all state that genuine levels sit above the ex-tax ones). The return base (D75): the engine uses `rate × opening RAV`, where handbook A1.143 averages the opening and discounted closing RAV within each year. Carried over verbatim into this model's end-of-year discounting, the averaged base breaks the annuity identity by a factor of (2+r)/(2(1+r)) — about 2.1% at the floor rate — because its NPV-neutrality in the real CFFM comes from intra-year receipt timing this model does not have. The handbook publishes no worked example, so the annuity identity (`level = RAV × AF` with every side block off) is the only available parity anchor, and the opening-RAV base is the unique choice that satisfies it exactly; the mini model states the base it actually uses rather than approximating the big model's.
